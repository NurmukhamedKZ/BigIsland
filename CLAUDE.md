...
- `Features/Awake/AwakeFeature.swift` — Amphetamine-like "don't sleep, even with the lid closed". Only `pmset -a disablesleep 1` survives lid close (IOPMAssertion/caffeinate don't), and it needs root: tries `sudo -n pmset …` first; otherwise one `osascript … with administrator privileges` prompt runs pmset and installs `/etc/sudoers.d/bigisland` (validated with `visudo -cf`) allowing only `pmset -a disablesleep 0|1` without a password. State is read from `pmset -g` (`SleepDisabled`), not stored. Turned off only in `stop()` (app quit); no battery limit (removed at user request).
