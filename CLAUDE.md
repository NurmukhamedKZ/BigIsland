...
- `Features/Layout/` — Punto Switcher–style RU⇄EN auto-switch, own code written after reading MIT projects (mikakostoev/layout-switcher, RuSwitcher, KeySwitcher). `LayoutRules.swift`: words are stored as key presses (`Key`) and read in any layout via `UCKeyTranslate` (`KeyLayout`); switch only if the typed reading is not a word and the other one is (`NSSpellChecker` + short-word / tech-word lists); if the dictionary knows neither, trigram statistics decide (`LayoutTrigrams.swift`, generated from hermitdave/FrequencyWords, gap > 40, ≥ 4 letters, never for Latin in code mode). `Typing` holds the word + up to 3 short words before it: when a word gets switched, the short ones that read as words in the target layout are fixed too («b ghbdtn» → «и привет»). In Russian layout `.` is «ю» and `@` is `"`, so file names with a known extension («СДФГВУюьв» → `CLAUDE.md`) and emails switch to English as a whole; digits with «ю» between them become a number («9ю5» → `9.5`) without switching the layout. Covered by `layoutSelfTest()` incl. whole phrases. `LayoutFeature.swift`: active `CGEventTap` on the main run loop (needs Accessibility), decides on Space, erases with backspaces, types the fix as unicode events marked with `eventSourceUserData`, then `TISSelectInputSource`. Code mode (bundle ids in `codeApps`, `com.jetbrains.*`; clicking the app name in the view toggles it per app, stored in `UserDefaults` `layout.codeOverrides`): Latin that looks like code (punctuation, camelCase, < 3 letters) is never turned into Russian; Return/Tab don't end a word. Double ⌥ converts the last word; undoing an auto-fix adds it to exceptions (`UserDefaults`). Only ru/en layouts; Kazakh and others are ignored. Secure input is skipped.
- `Features/Awake/AwakeFeature.swift` — Amphetamine-like "don't sleep, even with the lid closed". Only `pmset -a disablesleep 1` survives lid close (IOPMAssertion/caffeinate don't), and it needs root: tries `sudo -n pmset …` first; otherwise one `osascript … with administrator privileges` prompt runs pmset and installs `/etc/sudoers.d/bigisland` (validated with `visudo -cf`) allowing only `pmset -a disablesleep 0|1` without a password. State is read from `pmset -g` (`SleepDisabled`), not stored. Optional duration (∞/1/2/4/8 h, in-memory one-shot `Timer` → `toggle()`); otherwise turned off only in `stop()` (app quit); no battery limit (removed at user request).
- `Features/Mixer/MixerFeature.swift` — per-app volume/mute. Lists `kAudioHardwarePropertyProcessObjectList`, groups process objects by owner app (`responsibility_get_pid_responsible_for_pid` via dlsym, then bundle-id prefix: Chrome Helper / WebKit.GPU → the browser; processes without an `NSRunningApplication` — daemons like corespeechd — are hidden). Rows = apps with `IsRunningOutput` or a non-default setting. Refresh: listeners on process list + default output, plus a 1 Hz poll only while the view is visible. An app with volume ≠ 100 % or mute gets an `AppTap`: `CATapDescription(stereoMixdownOfProcesses:)` with `.mutedWhenTapped` + private aggregate device (default output as main subdevice) + IOProc copying the last input buffer × gain (Float32). No tap at 100 %. Tap is rebuilt when the app's process set or the default output changes. Needs `NSAudioCaptureUsageDescription`; min macOS 14.4. Settings are in-memory.
